namespace AccountManager.Core.Models.AppSettings
{
    public class RiotApiUri
    {
        public string? Auth { get; set; }
        public string? Valorant3rdParty { get; set; }
        public string? ValorantNA { get; set; }
        public string? ValorantAP { get; set; }
        public string? ValorantEU { get; set; }
        public string? Entitlement { get; set; }
        public string? LeagueNA { get; set; }
        public string? LeagueAP { get; set; }
        public string? LeagueEU { get; set; }
        public string? LeagueKR { get; set; }
        public string? LeagueBR { get; set; }
        public string? LeagueSessionUSW { get; set; }
        public string? LeagueSessionEUC { get; set; }
        public string? LeagueSessionAPSE { get; set; }
        public string? RiotCDN { get; set; }
    }
}
